...
$(document).ready(function () {
    ...
    RecordingModel = Backbone.Model.extend({
        openmirServer : 'http://openmir.sness.net:8000',
        urlRoot: 'http://openmir.sness.net:8000/api/v1/recording/',
        ...
        urlChange: function () {
            this.set("spectrogramUrl", "http://openmir.sness.net:8000/visualizations/spectrogram/" + this.id);
        },
    ...
    SpectrogramView = Backbone.View.extend({
        ...
        saveEvent: function(e,clips) {
            var collection = this.model.get("clips");

            newClips = []
            _.each(clips.clips, function(c) {
                var clip = new ClipModel(c);
                newClips.push(clip);
            });

            collection.set(newClips);
            var that = this;
        ...
        render: function() {
            ...
            $("#spectrogram").editorSpectrogramPlugin({
    ...
    WaveformView = Backbone.View.extend({
        ...
        render: function() {
            $("#waveform").editorWaveformPlugin({
                waveformUrl : this.model.get("waveformUrl"),
                recordingLengthSec : this.model.get("lengthSec"),
            });
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        initialize: function() {
            ...
            r.spectrogramUrl = "http://openmir.sness.net:8000/visualizations/spectrogram/" + window.recordingJson[0].pk;
            this.recording = new RecordingModel(r);

            this.spectrogramView = new SpectrogramView({model : this.recording});
            this.waveformView = new WaveformView({model : this.recording});
